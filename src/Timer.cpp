...
void Timer::SetFile(char * fname)

{

  if (strlen(fname) > DDS_FNAME_LEN)
...
void Timer::SetName(int no, char * name)
...

{
  ...
  gettimeofday(&userTimes0[no], NULL);
...

{
  ...
  int timeUser = (userTimes1[no].QuadPart -
 userTimes0[no].QuadPart);

#else

  gettimeofday(&userTimes1[no], NULL);
...

{
...

  {
   ...
   fprintf(fp, "%-14s %10d %10lld %10.2f %10.0f %10.2f\n",
      ...
      (double) userCum[no] / (double) count[no],

      1000. * systCum[no],
 
      1000. * (double) systCum[no] / (double) count[no]);
...

{
...

  {
    ...
    fprintf(fp, "%-14s %8lld %10lld %6.2f %4.1f %10.0f %6.2f %4.1f\n",
      ...
      AB_ucum,
      AB_ucum / (double) AB_ct,

      100. * AB_ucum / all_ucum,

      1000. * AB_scum,

      1000. * AB_scum / (double) AB_ct,
...
__int64 ct[TIMER_GROUPS];
...

  {
   ...
   fprintf(fp, "%-14s %8d %10lld %6.2f %4.1f %10.0f %6.2f %4.1f\n",
	 ...
	 AB_userCum[no] / (double) count[no],

         100. * AB_userCum[no] / AB_ucum,

         1000. * AB_systCum[no],

         1000. * AB_systCum[no] / (double) count[no],

         100. * AB_systCum[no] / (double) AB_scum);
