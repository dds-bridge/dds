...
/* 
   This object, Timer, was was written by Søren Hein.
...
/* 
 ...
 something that should be timed at multiple AB depths, i.e.

   cards played.  The first card of a new game is number 48,

   and the last card is number 0.
  ...
  For AB, first the
 times at depth-1 are subtracted out, and then the times for

   all calls at the same depth are subtracted out.  This still

   leaves the overhead of the timing itself.  As an approximation,

   there is one timing overhead left for each function, and it

   is on the order of the execution time of Evaluate(), which is
