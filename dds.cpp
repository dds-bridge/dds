...
/* DDS 2.5.3   A bridge double dummy solver.				      */
...
int Par(ddTableResults * tablep, parResults * presp,
	int vulnerable)
{
	/* vulnerable 0: None 1: Both 2: NS 3: EW */
	...
	for (int i = 0; i <= 1; i++)
	{
		if (sidesRes[i].contracts[0].underTricks > 0)
		{
			...
			for (k = 0; k < sidesRes[i].number; k++) {
				...
				strcat(presp->parContractsString[i], seats[sidesRes[i].contracts[k].seats]);
				sprintf(temp, "%d", sidesRes[i].contracts[k].level);
				buff[0] = static_cast<char>(cardSuit[denom_conv[sidesRes[i].contracts[k].denom]]);
				buff[1] = 'x';
				buff[2] = '\0';
				strcat(temp, buff);
			...
			for (k = 0; k < sidesRes[i].number; k++) {
				...
				int n = CalcMultiContracts(sidesRes[i].contracts[k].overTricks, sidesRes[i].contracts[k].overTricks
	...
	int vulnerable)
{
	...
	for (i = 0; i <= 1; i++)
	{
		...
		else
		{
			...
			for (m = 0; m < no_of_denom[i]; m++)
			{
				...
				CalcOverTricks(i, max_lower, best_par[m][i].par_tricks, m, sidesRes);

				sidesRes[i].contracts[m].level = best_par[m][i].par_tricks - 6 - sidesRes[i].contracts[m].overTricks;

			}
		}
	}

	/* Filter out par contracts where the other side has a higher par contract.
	This can happen when par scores differ for the two sides. */
	...
	for (int i = 0; i < 2; i++)
	{
		...
		while (k < sidesRes[opp_side[i]].number)
		{
			...
			if (((sidesRes[opp_side[i]].contracts[k].level + sidesRes[opp_side[i]].contracts[k].overTricks) > dom_level[opp_side[i]]) ||
				(((sidesRes[opp_side[i]].contracts[k].level + sidesRes[opp_side[i]].contracts[k].overTricks) == dom_level[opp_side[i]]) &&
				(j < dom_denom[opp_side[i]])))
			{
				if (((i == 0) && ((sidesRes[opp_side[i]].contracts[k].seats % 2) != 0)) ||
					((i == 1) && ((sidesRes[opp_side[i]].contracts[k].seats % 2) == 0)))
				{
					dom_denom[opp_side[i]] = j;
					dom_level[opp_side[i]] = sidesRes[opp_side[i]].contracts[k].level + sidesRes[opp_side[i]].contracts[k].overTricks;
				}
			}
			k++;
		}
	...
	if ((dom_denom[0] != -1) && (dom_denom[1] != -1))
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			for (k = 0; k < sidesRes[i].number; k++)
			{
				...
				if (((sidesRes[i].contracts[k].level + sidesRes[i].contracts[k].overTricks) < dom_level[opp_side[i]]) ||
					(((sidesRes[i].contracts[k].level + sidesRes[i].contracts[k].overTricks) == dom_level[opp_side[i]])
			...
			for (k = 0; k < sidesRes[i].number; k++)
			{
				...
				{
					sidesRes[i].contracts[m] = sidesRes[i].contracts[k];
					m++;
				}
			}
			sidesRes[i].number = m;
...
void SideSeats(int dr, int i, int t1, int t2, int order, parResultsMaster sidesRes[2])
{
...
void CalcOverTricks(int i, int max_lower, int tricks, int order, parResultsMaster sidesRes[2])
{
	switch (tricks - 6)
	{
	case 5:
