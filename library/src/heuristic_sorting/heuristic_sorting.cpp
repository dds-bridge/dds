...
int score_and_order(const HeuristicContext& context,
  ...
  const bool is_void = context.pos.length[hand_index][context.lead_suit] == 0;
...
void WeightAllocTrump0(const HeuristicContext& context,
  ...
  for (int i = 0; i < num_moves; ++i) {
    const int suit = scored_moves[i].move.card.suit;
    const unsigned short suit_count = context.pos.length[lead_hand][suit];
    const unsigned short suit_count_lho = context.pos.length[lho][suit];
    const unsigned short suit_count_rho = context.pos.length[rho][suit];
    ...
    int r_rank = context.rel_ranks->rank[scored_moves[i].move.card.rank];

    if ((suit != trump) &&
        (((context.pos.rank_in_suit[lho][suit] == 0) &&
          (context.pos.rank_in_suit[lho][trump] != 0)) ||
         ((context.pos.rank_in_suit[rho][suit] == 0) &&
          (context.pos.rank_in_suit[rho][trump] != 0)))) {
      suit_bonus = -12;
    }

    if ((suit != trump) && (context.pos.length[partner][suit] == 0) &&
        (context.pos.length[partner][trump] > 0) && (suit_count_rho > 0)) {
      suit_bonus += 17;
    }

    if ((context.pos.winner[suit].hand == rho) ||
        (context.pos.second_best[suit].hand == rho)) {
      if (suit_count_rho != 1) {
        suit_bonus += -12;
      }
    } else if ((context.pos.winner[suit].hand == lho) &&
               (context.pos.second_best[suit].hand == partner)) {
      if (context.pos.length[partner][suit] != 1) {
        suit_bonus += 27;
      }
    }

    if ((suit != trump) && (suit_count == 1) &&
        (context.pos.length[lead_hand][trump] > 0) &&
        (context.pos.length[partner][suit] > 1) &&
        (context.pos.winner[suit].hand == partner)) {
      suit_bonus += 19;
    }

    int suit_weight_delta = suit_bonus + suit_weight_d;

    if (context.pos.winner[suit].rank == scored_moves[i].move.card.rank) {
      if ((suit != trump)) {
        if ((context.pos.length[partner][suit] != 0) ||
            (context.pos.length[partner][trump] == 0)) {
          if (((context.pos.length[lho][suit] != 0) ||
               (context.pos.length[lho][trump] == 0)) &&
              ((context.pos.length[rho][suit] != 0) ||
               (context.pos.length[rho][trump] == 0))) {
            win_move = true;
          }
        } else if (((context.pos.length[lho][suit] != 0) ||
                    (context.pos.rank_in_suit[partner][trump] >
                     context.pos.rank_in_suit[lho][trump])) &&
                   ((context.pos.length[rho][suit] != 0) ||
                    (context.pos.rank_in_suit[partner][trump] >
                     context.pos.rank_in_suit[rho][trump]))) {
          win_move = true;
        }
      } else {
        win_move = true;
      }
    } else if (context.pos.rank_in_suit[partner][suit] >
               (context.pos.rank_in_suit[lho][suit] |
                context.pos.rank_in_suit[rho][suit])) {
      if (suit != trump) {
        if (((context.pos.length[lho][suit] != 0) ||
             (context.pos.length[lho][trump] == 0)) &&
            ((context.pos.length[rho][suit] != 0) ||
      ...
      if ((context.pos.length[partner][suit] == 0) &&
        ...
        if ((context.pos.length[lho][suit] == 0) &&
            ...
            (context.pos.length[rho][suit] == 0) &&
            (context.pos.length[rho][trump] != 0)) {
          if (context.pos.rank_in_suit[partner][trump] >
              (context.pos.rank_in_suit[lho][trump] |
               context.pos.rank_in_suit[rho][trump])) {
            win_move = true;
          }
        ...
        } else if ((context.pos.length[rho][suit] == 0) &&
                   (context.pos.length[rho][trump] != 0)) {
          if (context.pos.rank_in_suit[partner][trump] >
              context.pos.rank_in_suit[rho][trump]) {
            win_move = true;
    ...
    if (win_move) {
      if (((suit_count_lho == 1) && (context.pos.winner[suit].hand == lho)) ||
          ((suit_count_rho == 1) && (context.pos.winner[suit].hand == rho))) {
        scored_moves[i].weight = suit_weight_delta + 35 + r_rank;
      } else if (context.pos.winner[suit].hand == lead_hand) {
        if (context.pos.second_best[suit].hand == partner) {
          scored_moves[i].weight = suit_weight_delta + 48 + r_rank;
        } else if (context.pos.winner[suit].rank == scored_moves[i].move.card.rank) {
          scored_moves[i].weight = suit_weight_delta + 31;
        } else {
          scored_moves[i].weight = suit_weight_delta - 3 + r_rank;
        }
      } else if (context.pos.winner[suit].hand == partner) {
        if (context.pos.second_best[suit].hand == lead_hand) {
          ...
          scored_moves[i].weight = suit_weight_delta + 28 + r_rank;
                 ...
                 (scored_moves[i].move.card.rank == context.pos.second_best[suit].rank)) {
      ...
      if ((context.best_move.suit == suit) &&
          (context.best_move.rank == scored_moves[i].move.card.rank)) {
        scored_moves[i].weight += 55;
      } else if ((context.best_move_tt.suit == suit) &&
                 (context.best_move_tt.rank == scored_moves[i].move.card.rank)) {
        scored_moves[i].weight += 18;
      }
    } else {
      int third_best_hand = context.rel_ranks->abs_rank[3][suit].hand;

      if ((context.pos.second_best[suit].hand == partner) &&
          (partner == third_best_hand)) {
        suit_weight_delta += 20;
      } else if (((context.pos.second_best[suit].hand == lead_hand) &&
                  (partner == third_best_hand) &&
                  (context.pos.length[partner][suit] > 1)) ||
                 ((context.pos.second_best[suit].hand == partner) &&
      ...
      if (((suit_count_lho == 1) && (context.pos.winner[suit].hand == lho)) ||
        ...
        scored_moves[i].weight = suit_weight_delta + 17 - (scored_moves[i].move.card.rank);
      }
...
void WeightAllocNT0(const HeuristicContext& context,
  ...
  for (int i = 0; i < num_moves; ++i) {
    ...
    if (context.pos.winner[suit].rank == scored_moves[i].move.card.rank ||
        (context.pos.rank_in_suit[partner][suit] >
         (context.pos.rank_in_suit[lho][suit] |
          context.pos.rank_in_suit[rho][suit]))) {
      if (context.pos.second_best[suit].hand == rho) {
        if (suit_count_rho != 1) {
          suit_weight_delta += -1;
        }
      } else if (context.pos.second_best[suit].hand == lho) {
        if (suit_count_lho != 1) {
      ...
      if (((context.pos.second_best[suit].hand != lho) || (suit_count_lho == 1)) &&
          ((context.pos.second_best[suit].hand != rho) || (suit_count_rho == 1))) {
      ...
      } else if (scored_moves[i].move.card.rank == context.pos.second_best[suit].rank) {
        scored_moves[i].weight = suit_weight_delta + 48;
      } else {
        scored_moves[i].weight = suit_weight_delta + 29 - r_rank;
      }

      if ((context.best_move.suit == suit) && (context.best_move.rank == scored_moves[i].move.card.rank)) {
...
void WeightAllocTrumpNotvoid1(const HeuristicContext& context,
  ...
  const int max3rd = context.highest_rank[
                 context.pos.rank_in_suit[partner][lead_suit]];
  const int maxpd = context.highest_rank[
                 context.pos.rank_in_suit[rho][lead_suit]];
  ...
  for (int k = 0; k < num_moves; k++) {
    bool win_move = false;
    int r_rank = context.rel_ranks->rank[scored_moves[k].move.card.rank];

    if (lead_suit == trump) {
      if (maxpd > context.played_cards[0].rank && maxpd > max3rd)
        win_move = true;
      else if (scored_moves[k].move.card.rank > context.played_cards[0].rank &&
               scored_moves[k].move.card.rank > max3rd)
        win_move = true;
    } else {
      if (scored_moves[k].move.card.rank > context.played_cards[0].rank && scored_moves[k].move.card.rank > max3rd) {
        if ((max3rd != 0) ||
            (context.pos.length[partner][trump] == 0))
          win_move = true;
        else if ((maxpd == 0) && (context.pos.length[rho][trump] != 0) &&
                 (context.pos.rank_in_suit[rho][trump] >
                  context.pos.rank_in_suit[partner][trump]))
          win_move = true;
        ...
        if ((max3rd != 0) ||
          ...
          win_move = true;
                 ...
                 context.played_cards[0].rank > scored_moves[k].move.card.rank) {
        if ((maxpd == 0) && (context.pos.length[rho][trump] != 0)) {
          if ((max3rd != 0) ||
              (context.pos.length[partner][trump] == 0))
            win_move = true;
          else if (context.pos.rank_in_suit[rho][trump] >
                   context.pos.rank_in_suit[partner][trump])
            win_move = true;
        }
      } else if (maxpd == 0 && context.pos.length[rho][trump] != 0)
    ...
    if (win_move) {
      if (min3rd > scored_moves[k].move.card.rank)
        scored_moves[k].weight = 40 + r_rank;
      else if ((maxpd > context.played_cards[0].rank) &&
               (context.pos.rank_in_suit[lead_hand][lead_suit] >
                context.pos.rank_in_suit[rho][lead_suit]))
        scored_moves[k].weight = 41 + r_rank;
      else if (scored_moves[k].move.card.rank > context.played_cards[0].rank) {
        if (scored_moves[k].move.card.rank < maxpd)
          scored_moves[k].weight = 78 - (scored_moves[k].move.card.rank);
        else if (scored_moves[k].move.card.rank > max3rd)
          scored_moves[k].weight = 73 - (scored_moves[k].move.card.rank);
        else if (scored_moves[k].move.sequence)
          scored_moves[k].weight = 62 - (scored_moves[k].move.card.rank);
        else
          scored_moves[k].weight = 49 - (scored_moves[k].move.card.rank);
      } else if (maxpd > 0)
        scored_moves[k].weight = 47 - (scored_moves[k].move.card.rank);
      else
        scored_moves[k].weight = 40 - (scored_moves[k].move.card.rank);
    } else if (scored_moves[k].move.card.rank < min3rd || scored_moves[k].move.card.rank < minpd)
      scored_moves[k].weight = -9 + r_rank;
    else if (scored_moves[k].move.card.rank < context.played_cards[0].rank)
      scored_moves[k].weight = -16 + r_rank;
    else if (scored_moves[k].move.sequence)
      scored_moves[k].weight = 22 - (scored_moves[k].move.card.rank);
    else
      scored_moves[k].weight = 10 - (scored_moves[k].move.card.rank);
  }
}
...
void WeightAllocNTNotvoid1(const HeuristicContext& context,
  ...
  if (maxpd > context.played_cards[0].rank && maxpd > max3rd) {
    for (int k = 0; k < num_moves; k++)
      scored_moves[k].weight = -scored_moves[k].move.card.rank;
  } else {
    ...
    for (int k = 0; k < num_moves; k++) {
      ...
      if (scored_moves[k].move.card.rank > context.played_cards[0].rank && scored_moves[k].move.card.rank > max3rd)
        scored_moves[k].weight = 81 - scored_moves[k].move.card.rank;
      else if ((min3rd > scored_moves[k].move.card.rank) || (minpd > scored_moves[k].move.card.rank))
        ...
        scored_moves[k].weight = 13 - scored_moves[k].move.card.rank;
    }
  }
}
...
void WeightAllocTrumpVoid1(const HeuristicContext& context,
  ...
  for (int k = 0; k < num_moves; k++) {
    const int suit = scored_moves[k].move.card.suit;
    const unsigned short suit_count = context.pos.length[current_hand][suit];
    int suit_add;

    if (lead_suit == trump) {
      if (context.pos.rank_in_suit[rho][lead_suit] >
          (context.pos.rank_in_suit[partner][lead_suit] |
        ...
        if ((suit_count == 2) &&
            (context.pos.second_best[suit].hand == current_hand))
          suit_add += -4;
      }
      scored_moves[k].weight = -scored_moves[k].move.card.rank + suit_add;
    } else if (suit != trump) {
      if (context.pos.length[partner][lead_suit] != 0) {
        if (context.pos.rank_in_suit[rho][lead_suit] >
            (context.pos.rank_in_suit[partner][lead_suit] |
             (1 << (context.played_cards[0].rank - 2))))
          suit_add = 60 + (suit_count << 6) / 44;
        else if ((context.pos.length[rho][lead_suit] == 0) &&
                 (context.pos.length[rho][trump] != 0))
          suit_add = 60 + (suit_count << 6) / 44;
        else {
          ...
          if ((suit_count == 2) &&
            ...
            suit_add += -4;
                 ...
                 (context.pos.rank_in_suit[rho][trump] >
                  context.pos.rank_in_suit[partner][trump]))
        suit_add = 60 + (suit_count << 6) / 44;
      else if ((context.pos.length[partner][trump] == 0) &&
               (context.pos.rank_in_suit[rho][lead_suit] >
                (1 << (context.played_cards[0].rank - 2))))
        suit_add = 60 + (suit_count << 6) / 44;
      else {
      ...
      scored_moves[k].weight = -scored_moves[k].move.card.rank + suit_add;
    } else if (context.pos.length[partner][lead_suit] != 0) {
      suit_add = (suit_count << 6) / 44;
      scored_moves[k].weight = 24 - (scored_moves[k].move.card.rank) + suit_add;
    } else if ((context.pos.length[rho][lead_suit] == 0) &&
               (context.pos.length[rho][trump] != 0) &&
               (context.pos.rank_in_suit[rho][trump] >
                context.pos.rank_in_suit[partner][trump])) {
      ...
      if ((1 << (scored_moves[k].move.card.rank - 2)) >
          context.pos.rank_in_suit[partner][trump]) {
        suit_add = (suit_count << 6) / 44;
        scored_moves[k].weight = 24 - (scored_moves[k].move.card.rank) + suit_add;
      } else {
        ...
        if ((suit_count == 2) &&
            (context.pos.second_best[suit].hand == current_hand))
          suit_add += -4;
        scored_moves[k].weight = 15 - (scored_moves[k].move.card.rank) + suit_add;
      }
    }
  }
}
...
void WeightAllocNTVoid1(const HeuristicContext& context,
  ...
  if (context.pos.rank_in_suit[rho][lead_suit] >
    ...
    for (int k = 0; k < num_moves; k++) {
      const int suit = scored_moves[k].move.card.suit;
      unsigned short suit_count = context.pos.length[current_hand][suit];
      int suit_add = (suit_count << 6) / 23;
      if (suit_count == 2 && context.pos.second_best[suit].hand == current_hand)
        suit_add += -2;
      else if (suit_count == 1 && context.pos.winner[suit].hand == current_hand)
        suit_add += -3;
      scored_moves[k].weight = -scored_moves[k].move.card.rank + suit_add;
    }
  } else {
    for (int k = 0; k < num_moves; k++) {
      ...
      if ((suit_count == 2) &&
        ...
        suit_add += -6;
               ...
               (context.pos.winner[suit].hand == current_hand))
        suit_add += -8;
      scored_moves[k].weight = -scored_moves[k].move.card.rank + suit_add;
    }
  }
...
int RankForcesAce(const HeuristicContext& context, int cards4th, int num_moves, const ScoredMove* scored_moves) {
  ...
  if (secondRHO > context.played_cards[1].rank) {
    int k = 0;
    while (k < num_moves && scored_moves[k].move.card.rank > secondRHO)
      k++;
    ...
    int k = 0;
    while (k < num_moves && scored_moves[k].move.card.rank > context.played_cards[1].rank)
      k++;
...
void GetTopNumber(const HeuristicContext& context, int ris, int prank, int& topNumber, int& mno, int num_moves, const ScoredMove* scored_moves) {
  ...
  mno = 0;
  while (mno < num_moves - 1 && scored_moves[1 + mno].move.card.rank > prank)
    mno++;
...
void WeightAllocTrumpNotvoid2(const HeuristicContext& context,
  ...
  const int cards4th = context.pos.rank_in_suit[rho][lead_suit];
  const int max4th = context.highest_rank[cards4th];
  const int min4th = context.lowest_rank[cards4th];
  const int max3rd = scored_moves[0].move.card.rank;
  ...
  if (lead_suit == trump) {
    if (context.played_cards[1].rank < context.played_cards[0].rank && context.played_cards[0].rank > max4th) {
      ...
      for (int k = 0; k < num_moves; k++) {
        if (scored_moves[k].move.card.rank > max4th &&
            scored_moves[k].move.card.rank > context.played_cards[1].rank)
          scored_moves[k].weight = 58 - scored_moves[k].move.card.rank;
        else
          scored_moves[k].weight = -scored_moves[k].move.card.rank;
      }
    } else {
    ...
    if (max4th == 0) {
      ...
      for (int k = 0; k < num_moves; k++) {
        if (scored_moves[k].move.card.rank > context.played_cards[1].rank &&
            scored_moves[k].move.card.rank > max4th)
          scored_moves[k].weight = 60 - scored_moves[k].move.card.rank;
        else
          scored_moves[k].weight = -scored_moves[k].move.card.rank;
      }
    ...
    if (max4th == 0) {
      for (int k = 0; k < num_moves; k++) {
        if (scored_moves[k].move.card.rank > context.played_cards[1].rank)
          scored_moves[k].weight = 20 - scored_moves[k].move.card.rank;
        else
          scored_moves[k].weight = -scored_moves[k].move.card.rank;
      }
      return;
...
void WeightAllocNTNotvoid2(const HeuristicContext& context,
  ...
  if (context.played_cards[1].rank > context.played_cards[0].rank && context.played_cards[0].rank > max4th) {
    ...
    if (context.pos.length[lead_hand][lead_suit] == 0 &&
        context.pos.winner[lead_suit].hand == current_hand) {
      int oppLen = context.pos.length[rho][lead_suit] - 1;
      int lhoLen = context.pos.length[lho_d[lead_hand]][lead_suit];
      if (lhoLen > oppLen)
        oppLen = lhoLen;

      int topNumber, mno;
      GetTopNumber(context, context.pos.rank_in_suit[partner_d[lead_hand]][lead_suit],
...
void WeightAllocTrumpVoid2(const HeuristicContext& context,
  ...
  for (int k = 0; k < num_moves; k++) {
    ...
    if (context.played_cards[1].rank < context.played_cards[0].rank && context.played_cards[0].rank > max4th &&
        (max4th != 0 || context.pos.length[rho][trump] == 0)) {
      scored_moves[k].weight = -scored_moves[k].move.card.rank - 50;
      continue;
    }
    ...
    if (context.played_cards[1].suit == trump &&
      ...
      if (max4th != 0) {
        if (context.pos.second_best[lead_suit].hand == lead_hand) {
          suit_add = (suit_count << 6) / 50;
          scored_moves[k].weight = 36 - scored_moves[k].move.card.rank + suit_add;
        } else {
          suit_add = (suit_count << 6) / 50;
          scored_moves[k].weight = 48 - scored_moves[k].move.card.rank + suit_add;
        }
      } else if ((1 << (scored_moves[k].move.card.rank - 2)) >
                 context.pos.rank_in_suit[rho][trump]) {
        suit_add = (suit_count << 6) / 50;
        scored_moves[k].weight = 48 - scored_moves[k].move.card.rank + suit_add;
      } else {
        suit_add = (suit_count << 6) / 50;
        scored_moves[k].weight = -12 - scored_moves[k].move.card.rank + suit_add;
      }
    } else if (max4th != 0) {
      suit_add = (suit_count << 6) / 50;
      scored_moves[k].weight = 72 - scored_moves[k].move.card.rank + suit_add;
    } else if ((1 << (scored_moves[k].move.card.rank - 2)) >
               context.pos.rank_in_suit[rho][trump]) {
      suit_add = (suit_count << 6) / 50;
      scored_moves[k].weight = 48 - scored_moves[k].move.card.rank + suit_add;
...
void WeightAllocCombinedNotvoid3(const HeuristicContext& context,
  ...
  if (winning_suit == trump ||
    ...
    for (int k = 0; k < num_moves; k++) {
      if (scored_moves[k].move.card.rank > winning_rank)
        scored_moves[k].weight = 30 - scored_moves[k].move.card.rank;
      else
        scored_moves[k].weight = -scored_moves[k].move.card.rank;
    }
  }
...
void WeightAllocTrumpVoid3(const HeuristicContext& context,
  ...
  for (int k = 0; k < num_moves; k++) {
    const int suit = scored_moves[k].move.card.suit;
    const int mylen = context.pos.length[current_hand][suit];
    int val = (mylen << 6) / 24;
    if ((mylen == 2) && (context.pos.second_best[suit].hand == current_hand))
      val -= 2;

    if (lead_suit == trump) {
      scored_moves[k].weight = -scored_moves[k].move.card.rank + val;
    } else if (context.played_cards[2].rank > context.played_cards[0].rank && context.played_cards[2].rank > context.played_cards[1].rank) {
      if (suit == trump)
        scored_moves[k].weight = 2 - scored_moves[k].move.card.rank + val;
      else
        scored_moves[k].weight = 25 - scored_moves[k].move.card.rank + val;
    } else if (context.played_cards[2].suit == trump) {
      if (suit == trump) {
        int r_rank = context.rel_ranks->rank[scored_moves[k].move.card.rank];
        if (scored_moves[k].move.card.rank > context.played_cards[2].rank)
          scored_moves[k].weight = 33 + r_rank;
        else
          scored_moves[k].weight = -13 + r_rank;
      }
      else
        scored_moves[k].weight = 14 - (scored_moves[k].move.card.rank) + val;
    } else if (suit == trump) {
      int r_rank = context.rel_ranks->rank[scored_moves[k].move.card.rank];
      scored_moves[k].weight = 33 + r_rank;
    } else {
...
void WeightAllocNTVoid3(const HeuristicContext& context,
  ...
  for (int k = 0; k < num_moves; k++) {
    const int suit = scored_moves[k].move.card.suit;
    int mylen = context.pos.length[current_hand][suit];
    int val = (mylen << 6) / 27;
    if ((mylen == 2) && (context.pos.second_best[suit].hand == current_hand))
      val -= 6;
    else if ((mylen == 1) && (context.pos.winner[suit].hand == current_hand))
      val -= 8;
